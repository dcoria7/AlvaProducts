    ...
    <Link href={`/venue/${venue.id}`} className={`relative flex flex-col rounded-2xl overflow-hidden border border-gray-100 bg-white active:scale-95 transition-transform ${!isOpen ? "opacity-55" : ""}`}>
      <div className="relative w-full aspect-square bg-gray-100">
        {venue.media.logo_url ? (
          <Image src={venue.media.logo_url} alt={venue.name} fill sizes="(max-width: 768px) 50vw, 200px" className="object-cover" />
        ) : (
      ...
      <div className="p-2.5">
        ...
        {venue.location.is_ambulatory && <span className="text-[10px] text-amber-600 font-medium">Ambulante</span>}
      </div>
    </Link>
  );
}
...
export function VenueGrid() {
  ...
  useEffect(() => {
    getActiveVenues().then(setVenues).finally(() => setLoading(false));
  }, []);

  const filtered = venues.filter((v) => activeFilter === "all" || v.type === activeFilter || (activeFilter !== "both" && v.type === "both"));
  const open = filtered.filter((v) => v.status === "open");
  const closed = filtered.filter((v) => v.status === "closed");

  if (loading) return <div className="flex items-center justify-center py-24"><Loader2 className="w-8 h-8 animate-spin text-gray-300" /></div>;
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {TYPE_FILTERS.map((f) => (
            <button key={f.value} onClick={() => setActiveFilter(f.value)}
              className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${activeFilter === f.value ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {open.length > 0 && (
            <section>
              <h2 className="text-base font-bold text-emerald-600 mb-3">Abiertos</h2>
              <div className="grid grid-cols-2 gap-3">{open.map((v) => <VenueGridCard key={v.id} venue={v} />)}</div>
            </section>
          )}
          {closed.length > 0 && (
            <section>
              <h2 className="text-base font-bold text-gray-400 mb-3">Cerrados</h2>
              <div className="grid grid-cols-2 gap-3">{closed.map((v) => <VenueGridCard key={v.id} venue={v} />)}</div>
            </section>
          )}
        </>
      )}
