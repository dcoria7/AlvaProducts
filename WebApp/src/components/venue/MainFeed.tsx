...
function GridCard({ venue }: { venue: VenueWithDistance | Venue }) {
  ...
  return (
    <Link
      href={`/venue/${venue.id}`}
      className={`relative flex flex-col rounded-2xl overflow-hidden border border-gray-100 bg-white active:scale-95 transition-transform ${!isOpen ? "opacity-55" : ""}`}
    >
      ...
      <div className="px-2.5 py-2">
        ...
        {"distanceKm" in venue && venue.distanceKm < 9999 && (
          <p className="text-xs text-emerald-600 mt-0.5">
            {venue.distanceKm < 1 ? `${Math.round(venue.distanceKm * 1000)} m` : `${venue.distanceKm.toFixed(1)} km`}
          </p>
        )}
      </div>
    </Link>
...
export function MainFeed() {
  ...
  const withUpdate = venues.filter((v) => v.dailyUpdate?.text || v.dailyUpdate?.imageUrl);
          ...
          <div className="px-4 pt-5 pb-4">
            {open.length > 0 && (
              <section className="mb-5">
                <h2 className="text-sm font-bold text-emerald-600 mb-3">
