...
function WhatsAppButton({ phone, venueId, label = "Contactar por WhatsApp" }: { phone: string; venueId: string; label?: string }) {
  ...
  return (
    <button
      onClick={handleClick}
      className="flex items-center justify-center gap-2 w-full py-3.5 bg-[#25D366] text-white font-semibold rounded-2xl hover:bg-[#1ebe5d] transition-colors"
    >
    ...
    <section>
      ...
      <div className="flex flex-col gap-2">
        {services.map((service) => (
          <div key={service.id} className="flex items-center justify-between gap-3 p-3 bg-white rounded-xl border border-gray-100">
            <div className="flex-1 min-w-0">
  ...
  if (!venue) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 px-4 text-center">
        ...
        <Link href="/" className="text-sm text-emerald-600 font-medium">← Volver al inicio</Link>
      </div>
    );
  }
          ...
          <div className="flex items-start gap-2 text-sm text-gray-600">
            ...
            <div>
              {venue.location.address ? (
                <a
                  href={`https://maps.google.com/?q=${venue.location.coordinates.latitude},${venue.location.coordinates.longitude}`}
                  target="_blank"
              ...
              {venue.location.isAmbulatory && (
                <span className="ml-2 text-xs text-amber-600 font-medium">· Ambulante</span>
              )}
            </div>
          </div>
